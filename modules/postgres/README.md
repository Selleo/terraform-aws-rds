...
| <a name="input_performance_insights"></a> [performance\_insights](#input\_performance\_insights) | Performance insights configuration. Retention period valid values are 7, 731 (2 years) or a multiple of 31. | <pre>object({<br>    retention_period = 14<br>    kms_key_id       = null<br>  })</pre> | n/a | yes |
...
| <a name="input_version"></a> [version](#input\_version) | Postgres version. | `string` | `"14.5"` | no |
| <a name="input_vpc"></a> [vpc](#input\_vpc) | VPC configuration (id, CIDR that has access to RDS and subnet). | <pre>object({<br>    id     = string<br>    cidr   = string<br>    subnet = string<br>  })</pre> | n/a | yes |
